...
from orchestrator.autoencoder_dataset import Slakh2100_Pop909_Dataset, collate_fn
...
def batch_report(loss, n_epoch, idx, num_batch, mode='training', verbose=False):
    if verbose:
        ...
        print(f"\t KL loss [mix/trf]: {loss['kl_pno_red']:.3f}/{loss['kl_trf']:.3f}")
...
def train(model, dataloader, param_scheduler, optimizer_scheduler, writer_names, loss_writers, scheduler_writers, n_epoch):
    ...
    for idx, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        if (idx!=0) and (idx % (len(dataloader) // 10)) == 0:
            track_pad_mask = batch[5]
            mask = torch.logical_not(track_pad_mask.reshape(-1))
            batch_z = model.function_enc.batch_z[mask]
            batch_z = batch_z.reshape(-1, batch_z.shape[-1])
            code_usage, code_dead = model.function_enc.vq_quantizer.random_restart(batch_z)
            model.function_enc.vq_quantizer.reset_usage()
