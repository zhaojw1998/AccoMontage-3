...
class ChordDecoder(nn.Module):
    ...
    def chord_loss(self, c, recon_root, recon_chroma, recon_bass):
        ...
        root_loss = self.loss_fun(recon_root, root)
        chroma_loss = self.loss_fun(recon_chroma, chroma)
        bass_loss = self.loss_fun(recon_bass, bass)
...
class TextureEncoder(nn.Module):
    ...
    def forward(self, pr):
        ...
        pr = self.fc2(torch.relu(self.fc1(pr)))  # (bs, 8, emb_size)
        pr = self.gru(pr)[-1]
...
class PianoTreeDecoder(nn.Module):
    ...
    def __init__(self, device=None, note_embedding=None,
        ...
        self.pitch_loss_func = nn.CrossEntropyLoss(ignore_index=self.pitch_pad)
        self.dur_loss_func = nn.CrossEntropyLoss(ignore_index=self.dur_pad)
    ...
    def recon_loss(self, x, recon_pitch, recon_dur, weights=(1, 0.5),
        ...
        pitch_loss = self.pitch_loss_func(recon_pitch, gt_pitch)

        if not weighted_dur:
            recon_dur = recon_dur.view(-1, 2)
            gt_dur = x[:, :, 1:, 1:].contiguous().view(-1)
            dur_loss = self.dur_loss_func(recon_dur, gt_dur)
        else:
            recon_dur = recon_dur.view(-1, self.dur_width, 2)
            gt_dur = x[:, :, 1:, 1:].contiguous().view(-1, self.dur_width)
            dur0 = self.dur_loss_func(recon_dur[:, 0, :], gt_dur[:, 0])
            dur1 = self.dur_loss_func(recon_dur[:, 1, :], gt_dur[:, 1])
            dur2 = self.dur_loss_func(recon_dur[:, 2, :], gt_dur[:, 2])
            dur3 = self.dur_loss_func(recon_dur[:, 3, :], gt_dur[:, 3])
            dur4 = self.dur_loss_func(recon_dur[:, 4, :], gt_dur[:, 4])
...
class PolyDisVAE(nn.Module):
    ...
    def __init__(self, device, chd_size=256, voi_size=256, txt_size=256,
                   num_channel=10):
        super(PolyDisVAE, self).__init__()
        
        self.chd_encoder = ChordEncoder(36, 1024, chd_size)
        self.voi_encoder = TextureEncoder(256, 1024, voi_size, num_channel)
        self.txt_encoder = TextureEncoder(256, 1024, txt_size, num_channel)

        self.pnotree_decoder = PianoTreeDecoder(device = device, \
                                                dec_dur_hid_size=64, \
                                                z_size=chd_size + voi_size + txt_size)
        self.voi_decoder = PianoTreeDecoder(device = device, \
                                                num_step=8, \
                                                dur_width=3, \
                                                z_size=chd_size + voi_size)
        self.chd_decoder = ChordDecoder(z_dim=chd_size, \
                                        num_step=self.pnotree_decoder.num_step)
        
        self.device = device


    def inference_encode(self, pno, voi, chd):
        self.eval()
        with torch.no_grad():
            dist_txt = self.txt_encoder(pno)
            dist_voi = self.voi_encoder(voi)
            dist_chd = self.chd_encoder(chd)
        return dist_txt, dist_voi, dist_chd

    def inference_decode(self, z_chd, z_voi, z_txt):
        self.eval()
        with torch.no_grad():
            
            pitch_outs, dur_outs = self.voi_decoder(torch.cat([z_chd, z_voi], dim=-1), True, None, None, 0., 0.)
            est_x_voi, _, _ = self.voi_decoder.output_to_numpy(pitch_outs, dur_outs)

            pitch_outs, dur_outs = self.pnotree_decoder(torch.cat([z_chd, z_voi, z_txt], dim=-1), True, None, None, 0., 0.)
            est_x_pno, _, _ = self.pnotree_decoder.output_to_numpy(pitch_outs, dur_outs)

        return est_x_pno, est_x_voi

    def inference(self, pno, chd, sample=False, return_voi=False):
        self.eval()
        with torch.no_grad():
            #if voi is None:
            voi = self.pr2voice(pno)
            dist_txt, dist_voi, dist_chd = self.inference_encode(pno, voi, chd)
            z_txt, z_voi, z_chd = get_zs_from_dists([dist_txt, dist_voi, dist_chd], sample)
            est_x_pno, est_x_voi = self.inference_decode(z_chd, z_voi, z_txt)
        if return_voi:
            return est_x_pno, est_x_voi
        else:
            return est_x_pno
    
    def pr2voice(self, pr_mat, res=4):
        voice = torch.zeros(pr_mat.shape).to(pr_mat.device)
        for b, t, p in zip(*torch.nonzero(pr_mat, as_tuple=True)):
            t_qant = t // res * res
            #dur_quant = math.ceil(pr_mat[t, p]  / res) * res
            voice[b, t_qant: t_qant + res, p] = 1
        return voice.float()

    def loss(self, pno, voi, chd, pno_grid, voi_grid, tfr1=0., tfr2=0., tfr3=0., beta=0.1, weights=(1, 0.5)):
        #print(pr_mat.shape, dt_x.shape)
        outputs = self.run(pno, voi, chd, pno_grid, voi_grid, tfr1, tfr2, tfr3)
        loss = self.loss_function(pno_grid, voi_grid, chd, *outputs, beta, weights)
        return loss

    def run(self, pno, voi, chd, pno_grid, voi_grid, tfr1, tfr2, tfr3):
        grid_pno, lengths_pno = self.pnotree_decoder.emb_x(pno_grid)
        grid_voi, lengths_voi = self.voi_decoder.emb_x(voi_grid)
        
        dist_txt = self.txt_encoder(pno)
        dist_voi = self.voi_encoder(voi)
        dist_chd = self.chd_encoder(chd)
        
        z_chd, z_voi, z_txt = get_zs_from_dists([dist_chd, dist_voi, dist_txt], sample=True)
        #dec_z = torch.cat([z_chd, z_txt], dim=-1)
        
        
        
        recon_root, recon_chroma, recon_bass = self.chd_decoder(z_chd, \
                                                                inference = False, \
                                                                tfr = tfr3, \
                                                                c = chd)
        
        voi_pitch_outs, voi_dur_outs = self.voi_decoder(torch.cat([z_chd, z_voi], dim=-1), \
                                                    inference = False, \
                                                    x = grid_voi, \
                                                    lengths = lengths_voi, \
                                                    teacher_forcing_ratio1 = tfr1, \
                                                    teacher_forcing_ratio2 = tfr2)
        
        pno_pitch_outs, pno_dur_outs = self.pnotree_decoder(torch.cat([z_chd, z_voi, z_txt], dim=-1), \
                                                    inference = False, \
                                                    x = grid_pno, \
                                                    lengths = lengths_pno, \
                                                    teacher_forcing_ratio1 = tfr1, \
        ...
        return pno_pitch_outs, pno_dur_outs, voi_pitch_outs, voi_dur_outs, \
                dist_chd, dist_voi, dist_txt, \
                recon_root, recon_chroma, recon_bass
    
    def loss_function(self, pno, voi, chd, recon_pno_pitch, recon_pno_dur, recon_voi_pitch, recon_voi_dur, \
                      dist_chd, dist_voi, dist_txt, recon_root, recon_chroma, recon_bass, \
                      beta, weights, weighted_dur=False):
        pno_loss, pno_pl, pno_dl = self.pnotree_decoder.recon_loss(pno, recon_pno_pitch, recon_pno_dur, weights, weighted_dur)
        voi_loss, voi_pl, voi_dl = self.voi_decoder.recon_loss(voi, recon_voi_pitch, recon_voi_dur, weights, weighted_dur)
        kl_loss, kl_chd, kl_voi, kl_txt = self.kl_loss(dist_chd, dist_voi, dist_txt)
        chord_loss, root, chroma, bass = self.chd_decoder.chord_loss(chd, recon_root, recon_chroma, recon_bass)
        loss = pno_loss + voi_loss + chord_loss + beta * kl_loss
        return loss, pno_loss, pno_pl, pno_dl, voi_loss, voi_pl, voi_dl, \
                kl_loss, kl_chd, kl_voi, kl_txt, \
                chord_loss, root, chroma, bass 
    
    #def kl_loss(self, *dists):
    #    # kl = kl_with_normal(dists[0])
    #    kl_chd = kl_with_normal(dists[0])
    #    kl_voi = kl_with_normal(dists[1])
    #    kl_txt = kl_with_normal(dists[2])
    #    kl_loss = kl_chd + kl_voi + kl_txt
    #    return kl_loss, kl_chd, kl_voi, kl_txt
