...
class PianoTreeDecoder(nn.Module):
        ...
        with torch.no_grad():
            ...
            out = torch.zeros(
                [ind_x.size(0) * self.num_step * self.max_simu_note,
                 self.pitch_range + 1],
                dtype=torch.float).to(ind_x.device)
    ...
    def pitch_dur_ind_to_note_token(self, pitch_inds, dur_inds, batch_size):
        ...
        token = token.to(pitch_inds.device)
    ...
    def decode_notes(self, notes_summary, batch_size, notes, inference,
        ...
        if inference:
            ...
            sos = self.get_sos_token().to(notes_summary_hid.device)  # (note_size,)
        ...
        predicted_notes = predicted_notes.to(notes_summary_hid.device)

        lengths = lengths.to(notes_summary_hid.device)
