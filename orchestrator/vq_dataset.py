...
class VQ_LMD_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        fp = self.func_pitch_list[song_id][start: min(total_len, start+SAMPLE_LEN)]
        ft = self.func_time_list[song_id][start: min(total_len, start+SAMPLE_LEN)]
        return mix, prog, fp, ft, (start, total_len)
    ...
    def load_lmd(self):
        ...
        for song in tqdm(lmd_list):
            ...
            fp = lmd_data['func_pitch']    #(num2bar, track)
            ft = lmd_data['func_time']    #(num2bar, track, 8)
            ...
            self.func_pitch_list.append(fp)
            self.func_time_list.append(ft)
...
def collate_fn(batch, device):
    ...
    for mix, prog, fp, ft, (start, total_len) in batch:
        #print(mix.shape, prog.shape, fp.shape, ft.shape, start, total_len)
        ...
        #print('prog', len(prog), 'max_track', max_tracks)
        if len(prog) < max_tracks:
            prog = np.pad(prog, ((0, max_tracks-len(prog))), mode='constant', constant_values=(NUM_INSTR_CLASS,))
            fp = np.pad(fp, ((0, 0), (0, max_tracks-fp.shape[1])), mode='constant', constant_values=(NUM_PITCH_CODE,))
            ft = np.pad(ft, ((0, 0), (0, max_tracks-ft.shape[1]), (0, 0)), mode='constant', constant_values=(NUM_TIME_CODE,))
            #print('fp pad', fp.shape)
        if len(mix) < max_dur:
            ...
            fp = np.pad(fp, ((0, max_dur-len(fp)), (0, 0)), mode='constant', constant_values=(NUM_PITCH_CODE,))
            ft = np.pad(ft, ((0, max_dur-len(ft)), (0, 0), (0, 0)), mode='constant', constant_values=(NUM_TIME_CODE,))
            
        #print('fp', fp.shape, max_tracks)
        mixture.append(mix)
        programs.append(prog)
        func_pitch.append(fp)
        func_time.append(ft)
    ...
    return torch.from_numpy(np.array(mixture)).float().to(device), \
            ...
            torch.from_numpy(np.array(func_pitch)).long().to(device), \
            torch.from_numpy(np.array(func_time)).long().to(device), \
